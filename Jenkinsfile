pipeline {
    ...
    stages {
        stage('Check') {
            steps {
                git branch: 'develop',credentialsId:'0-shingo', url:'https://github.com/Spharos-final-project-WOOYANO/Reservaton'
            }
        }
        stage('Build'){
            steps{
        ...
        stage('Deploy'){
            steps{
                sh 'docker run -d --name reservaton-service -p 8761:8000 reservation-service-img'
            }
        }
    }
}
